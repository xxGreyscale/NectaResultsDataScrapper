...
from common.Domain.result import NectaACSEEResult, ACSEESubjectAndGrade, NectaCSEEResult, CSEESubjectAndGrade
...
class ResultMapper:
    def acsee_result_mapping(self, center_no: str, necta_raw_results: list, year) -> list[NectaACSEEResult]:
        try:
    ...
    def csee_result_mapping(self, center_no: str, necta_raw_results: list, year) -> list[NectaCSEEResult]:
        try:
            return list(map(lambda result: NectaCSEEResult(
                year=year,
                index_number=result["CNO"].split("/")[1] if len(result.get("CNO").split("/")) > 1 else result.get("CNO"),
                exam_center=center_no,
                division=DivisionEnum.from_string(result.get("DIV")) if result.get("DIV") is not None else None,
                sex=SexEnum(result.get("SEX")) if result.get("SEX") in ("M", "F", "None") else SexEnum.NONE,
                aggregate=result.get("AGGT", 0),
                subjects=self.csee_subject_grade_mapping(result.get("DETAILED SUBJECTS")),
            ), necta_raw_results))
        except Exception as e:
            raise ValueError(f"Error mapping NECTA results: {e}")
    ...
    @staticmethod
    def csee_subject_grade_mapping(subject_grades: dict) -> list:
        """"
        This function maps the subject grades to a dictionary
        :param subject_grades: The subject grade dictionary
        :return: A dictionary with subject names as keys and grades as values
        ...
        for key, value in subject_grades.items() if subject_grades is not None else {}.items():
            ...
            try:
                subject = CSEESubjectEnum.from_value_or_abbr(normalized_key)
            except ValueError:
                unknown_subjects.add(key)
                continue
            result.append(CSEESubjectAndGrade(subject=subject,
                                              grade=GradeEnum.from_value_or_abbr(value)))
        if unknown_subjects:
            ResultMapper._record_unknown_csee_subjects(unknown_subjects)
        return result

    @staticmethod
    def _record_unknown_csee_subjects(unknown_subjects: set[str]) -> None:
        file_path = "/Users/salinastic/PycharmProjects/necta/resource/csv/csee_unknown_subjects.txt"
        try:
            with open(file_path, "a", encoding="utf-8") as handle:
                for subject in sorted(unknown_subjects):
                    handle.write(subject + "\n")
        except Exception:
            # Avoid masking the original error with a file IO issue.
            pass
    ...
    @staticmethod
    def csee_to_document(domain: NectaCSEEResult) -> ResultDocument:
        return ResultDocument(
            identifiers=ResultDocumentIdentifiers(index_number=domain.exam_center + "/" + domain.index_number) if domain.index_number and domain.exam_center else None,
            current=CurrentResultDocument(
                year=domain.year,
    ...
    @staticmethod
    def csee_from_document(document: ResultDocument) -> NectaCSEEResult:
